...
class WallpaperClockFinder(QObject):
            ...
            for wp in files:
                if wp.isDir():
                    ...
                    if (name == '.') or (name == '..'):
                        ...
                        self.paths.append(wp.filePath())
                elif wp.suffix().toLower() in WallpaperClockModel.ValidSuffixes and \
...
class WallpaperClockModel(QAbstractListModel):
    ...
    def addBackground(self, path):
        ...
        if not self.contains(path) and QFile.exists(path) and \
           wp.suffix().toLower() in WallpaperClockModel.ValidSuffixes:
    ...
    def findAllBackgrounds(self, structureParent, container, p):
        ...
        finder = WallpaperClockFinder(structureParent, container, p, localEventLoop)
